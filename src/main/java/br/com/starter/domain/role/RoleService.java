...
public class RoleService {
    ...
    public Role createRole(String name, List<UUID> privilegeIds) {
        if (roleRepository.existsByName(name)) {
            throw new FrontDisplayableException(HttpStatus.CONFLICT, "Role já existe com este nome!");
        }
    ...
    public Role addPrivilegesToRole(UUID roleId, List<UUID> privilegeIds) {
        ...
        if (validPrivileges.isEmpty()) {
            throw new FrontDisplayableException(HttpStatus.BAD_REQUEST, "Nenhum dos privilégios fornecidos foi encontrado!");
        }
    ...
    public boolean deleteRole(UUID roleId) {
        if (!roleRepository.existsById(roleId)) {
            throw new FrontDisplayableException(HttpStatus.NOT_FOUND, "Role não encontrada!");
        }
